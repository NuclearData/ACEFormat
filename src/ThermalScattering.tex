...
\begin{XSSTable}{Start of \Block{ITXE} for $\nxs{7}=2$}
  \startblock{ITXE}          & \var{L}$(l),l=1,\ldots,N_{in}$  & Location in \XSS\ of distribution for incident energy $l$ \\
  \startblock{ITXE}+$N_{in}$ & $N_E(l),l=1,\ldots,N_{in}$      & Number of outgoing energies for incident energy $l$
...
\begin{XSSTable}{\Block{ITXE} for $\nxs{7}=2$ (continued)}
  ...
  \multicolumn{1}{c}{\vdots}       & \multicolumn{1}{c}{\vdots}                 & \multicolumn{1}{c}{\vdots}                                                    \\
  \var{L}(1)+1+($N'$-1)(3+$N_\mu$) & $E_{N'}^{out}[E_{in}(1)]$                     & Last of \var{NIEB} outgoing energies for inelastic scattering at $E_{in}(1)$    \\
  \var{L}(1)+2+($N'$-1)(3+$N_\mu$) & $\mathrm{PDF}_{N'}[E_{in}(1)]$          & Probability density function value for $E_{N'}^{out}[E_{in}(1)]$ \\
  \var{L}(1)+3+($N'$-1)(3+$N_\mu$) & $\mathrm{CDF}_{N'}[E_{in}(1)]$          & Cumulative distribution function value for $E_{N'}^{out}[E_{in}(1)]$ \\
  \var{L}(1)+4+($N'$-1)(3+$N_\mu$) & $\mu_l(1\rightarrow N'), l=1,\ldots,N_\mu$  & Discrete cosines for scattering from $E_{in}(1)$ to $E_{N'}^{out}[E_{in}(1)]$    \\
  \midrule
  \multicolumn{3}{l}{(Repeat for all remaining values of $E_{in}$)}
