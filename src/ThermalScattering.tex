...
\begin{enumerate}
  ...
  \item \textbf{\Block{ITCE}} and \textbf{\Block{ITCE2}}---contains the energy-dependent elastic scattering cross sections.
                                The \Block{ITCE} exists if the material has coherent and/or incoherent elastic scattering
                                (\nxs{5}$\neq0$ and \jxs{4}$\neq0$). The \Block{ITCE2} only exists for mixed mode elastic scattering
  ...
  \item \textbf{\Block{ITCA}} and \textbf{\Block{ITCA2}}---contains angular distributions for elastic scattering.
                                The \Block{ITCA} exists if the material has coherent and/or incoherent elastic scattering
                                (\nxs{5}$\neq0$, \jxs{6}$\neq0$ and \nxs{6}$\neq-1$). The \Block{ITCE2} only exists for mixed mode
...
\begin{NXSTable}{thermal scattering}
  ...
  8  & \var{NCL2}  & Elastic dimensioning parameter for the second elastic block \\
     & \ldots      & \\
  16 & ---         &
...
\begin{JXSTable}{thermal scattering}
  ...
  7  & \var{ITCE2} & Elastic energy table (used for incoherent elastic scattering if \nxs{5}=5)                \\
  8  & \var{ITCX2} & Elastic cross sections (used for incoherent elastic scattering if \nxs{5}=5)              \\
  9  & \var{ITCA2} & Elastic angular distributions (used for incoherent elastic scattering if \nxs{5}=5)       \\
     & \ldots      & \\
  32 & ---         &
...
The format of the \Block{ITCE} and \Block{ITCE2} is given in \Tableref{tab:ITCEBlock}. The index at the start of the \Block{ITCE}
and \Block{ITCE2} is respectively \startblock{ITCE}=\jxs{4} and \startblock{ITCE}=\jxs{7}.
...
\subsubsection{\textsf{ITCA} Block}\label{sec:ITCABlock}\label{sec:ITCA2Block}

The format of the \Block{ITCA} and \Block{ITCA2} is given in \Tableref{tab:ITCABlock}. The index at the start of the \Block{ITCA}
and \Block{ITCA2} is respectively \startblock{ITCE}=\jxs{6} and \startblock{ITCE}=\jxs{9}. For each incident energy from the
\Block{ITCE} and \Block{ITCE2} respectively, $N_\mu=$\nxs{6}+1 and $N_\mu=$\nxs{8}+1 discrete cosines are given.
