...
\begin{table}[H] \centering
  ...
  \begin{tabular}{rll}
    ...
    5       & IDPNC  & Elastic scattering mode (coherent=4, incoherent$\ne$4)     \\
...
For incoherent elastic scattering (\nxs{5}$\neq4$), $\sigma_{el}(E_{el}(l)) = P(l)$ with linear-linear interpolation between points. For coherent elastic scattering (\nxs{5}$=4$), $\sigma_{el}(E)=P(l)/E$ for $E_{el}(l) \le E < E_{el}(l+1)$. Note that in this case, $\sigma_{el}(E)=0$ below $E_{el}(1)$. However, above $E_{el}(N_{el})$, $\sigma_{el}(E) = P(N_{el})/E$.
...
\begin{BlockTable}{ITXE}
  \startblock{ITXE}                       & $E_1^{out}[E_{in}(1)]$                     & First of \nxs{4} outgoing energies for inelastic scattering at $E_{in}(1)$    \\
  \startblock{ITXE}+1                     & $\mu_l(1\rightarrow 1), l=1,\ldots,N_\mu$  & Discrete cosines for scattering from $E_{in}(1)$ to $E_1^{out}[E_{in}(1)]$    \\
  \startblock{ITXE}+1+$N_\mu$             & $E_2^{out}[E_{in}(1)]$                     & Second of \nxs{4} outgoing energies for inelastic scattering at $E_{in}(1)$   \\
  ...
  \startblock{ITXE}+($N'$-1)(1+$N_\mu$)   & $E_{N'}^{out}[E_{in}(1)]$                  & Second of \nxs{4} outgoing energies for inelastic scattering at $E_{in}(1)$   \\
