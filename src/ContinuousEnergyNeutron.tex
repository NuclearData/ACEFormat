...
\begin{enumerate}
  ...
  \item \textbf{\Block{LQR}}---contains a list of kinematic $Q$-values for all neutron reactions other than elastic scattering. The \Block{LQR} exists if \nxs{4}$\neq0$. See \Sectionref{sec:LQRBlock}.
  ...
  \item \textbf{\Block{PTYPE}}---contains a list of particle types for which production data will be given. The \Block{PTYPE} exists if \jxs{30}$\neq0$. See \Sectionref{sec:PTYPEBlock}.
  \item \textbf{\Block{NTRO}}---contains the number of reactions that produce the corresponding particle type given in the \Block{PTYPE}. The \Block{NTRO} exists if \jxs{31}$\neq0$. See \Sectionref{sec:NTROBlock}.
  \item \textbf{\Block{IXS}}---particle production data locators for each particle type given in the \Block{PTYPE}. The \Block{IXS} exists if \jxs{32}$\neq 0$. See \Sectionref{sec:IXSBlock}.
  \item \textbf{\Block{HPD}}---total particle production cross section and average heating numbers for the current particle type. The \Block{HPD} for a given particle type $i$ exists if $\jxs{32}+10*(i-1) \neq 0$. See \Sectionref{sec:HPDBlock}.
  \item \textbf{\Block{MTRH}}---contains a list of ENDF MT numbers for all reactions that produce the current particle. The \Block{MTR} for a given particle type $i$ exists if $\jxs{32}+10*(i-1) + 1 \neq 0$. See \Sectionref{sec:MTRHBlock}.
  \item \textbf{\Block{TYRH}}---contains the reaction types for all reactions that produce the current particle. The \Block{TYRH} for a given particle type $i$ exists if $\jxs{32}+10*(i-1) + 2 \neq 0$. See \Sectionref{sec:TYRHBlock}.
  \item \textbf{\Block{LSIGH}}---contains the cross section locators for all reactions that produce the current particle. The \Block{LSIGH} for a given particle type $i$ exists if $\jxs{32}+10*(i-1) + 3 \neq 0$. See \Sectionref{sec:LSIGHBlock}.
  \item \textbf{\Block{SIGH}}---contains the cross section data for all reactions that produce the current particle. The \Block{SIGH} for a given particle type $i$ exists if $\jxs{32}+10*(i-1) + 4 \neq 0$. See \Sectionref{sec:SIGHBlock}.
  \item \textbf{\Block{LANDH}}---contains the angular distribution locators for all reactions that produce the current particle. The \Block{LANDH} for a given particle type $i$ exists if $\jxs{32}+10*(i-1) + 5 \neq 0$. See \Sectionref{sec:LANDHBlock}.
  \item \textbf{\Block{ANDH}}---contains the angular distribution data for all reactions that produce the current particle. The \Block{ANDH} for a given particle type $i$ exists if $\jxs{32}+10*(i-1) + 6 \neq 0$. See \Sectionref{sec:ANDHBlock}.
  \item \textbf{\Block{LDLWH}}---contains the energy distribution locators for all reactions that produce the current particle. The \Block{LDLWH} for a given particle type $i$ exists if $\jxs{32}+10*(i-1) + 7 \neq 0$. See \Sectionref{sec:LDLWHBlock}.
  \item \textbf{\Block{DLWH}}---contains the energy distribution data for all reactions that produce the current particle. The \Block{DLWH} for a given particle type $i$ exists if $\jxs{32}+10*(i-1) + 8 \neq 0$. See \Sectionref{sec:DLWHBlock}.
  \item \textbf{\Block{YH}}---contains the particle production yield multiplier for all reactions that produce the current particle. The \Block{YH} for a given particle type $i$ exists if $\jxs{32}+10*(i-1) + 9 \neq 0$. See \Sectionref{sec:DLWHBlock}.
\end{enumerate}
...
\begin{ThreePartTable}
  ...
  \begin{NXSTable}{continuous-energy neutron}
      ...
      7  & \var{NTYPE} & Number of particle types for which production data is given \\
...
\begin{JXSTable}{continuous-energy neutron}
    ...
    22 & \var{END}   & Last word of the conventional table (last word of photon production data) \\
    ...
    27 & \var{DNED}  & Delayed neutron energy distributions \\
       & \ldots      & \\
    30 & \var{PTYPE} & Particle type array \\
    31 & \var{NTRO}  & Array containing the number of particle production reactions \\
    32 & \var{NEXT}  & Table of particle production locators (IXS array)
...
\subsubsection{\textsf{NU} and \textsf{DNU} Blocks}\label{sec:NUBlock}\label{sec:DNUBlock}
...
The \Block{BDD} is used to specify basic delayed neutron precursor data and is present only if \jxs{25} > 0. For every precursor group (the total number of precursor groups is given in \nxs{8}), a decay constant is given along with the partial probability that a delayed fission neutron is born from the current group. This data is given in the format given in table \Tableref{tab:DelayedPrecursorDistribution}. The starting index \var{BDD} for this block is given by \jxs{25}.
...
\begin{ThreePartTable}
  ...
  \multicolumn{3}{c}{\bfseries Data for precursor group 1} \\
    $\var{BDD}$                 & \var{DEC}$_{1}$                  & Decay constant for the group 1 \\
    $\var{BDD}+1$               & $N_{R}$                          & Number of interpolation regions \\
    $\var{BDD}+2$               & \var{NBT}$(l), l=1,\ldots,N_{R}$ & ENDF interpolation parameters\tnotex{tn:schemeDelayedPrecursors} \\
    $\var{BDD}+2+N_{R}$         & \var{INT}$(l), l=1,\ldots,N_{R}$ & ENDF interpolation scheme \\
...
\subsubsection{\textsf{MTR}, \textsf{MTRP} and \textsf{MTRH} Blocks}\label{sec:MTRBlock}\label{sec:MTRPBlock}\label{sec:MTRHBlock}

The format of the \Block{MTR} (for incident neutron reactions), \Block{MTRP} (for photon production reactions) and \Block{MTRH} (for particle production reactions), is given in \Tableref{tab:MTRBlock} and provides a list of \MT\ numbers for which data is available in other blocks of the ACE Table. The starting index depends on whether it is the \Block{MTR}, \Block{MTRP} or \Block{MTRH} and are given in \Tableref{tab:LMT_NMT}. For the particle production \Block{MTRH}, \var{i} refers to the index of the corresponding particle type defined on the \Block{PTYPE} and is between 1 and \var{NTYPE}.
...
\begin{table}[h!] \centering
  \begin{tabular}[h]{lll}
    ...
    \bottomrule
  \end{tabular}
  \caption{\var{LMT} and \var{NMT} values for the \Block{MTR} and \Block{MTRH}.}
  \label{tab:LMT_NMT}
\end{table}
...
\subsubsection{\textsf{TYR} and \textsf{TYRH} Blocks}\label{sec:TYRBlock}\label{sec:TYRHBlock}

The format of the \Block{TYR} (for incident neutron reactions) and \Block{TYRH} (for particle production reactions) is given in \Tableref{tab:TYRBlock}. The starting index \var{LTYR} depends on whether it is the \Block{TYR} or \Block{TYRH} and is given in \Tableref{tab:TYR_NMT}. For the particle production \Block{TYRH}, \var{i} refers to the index of the corresponding particle type defined on the \Block{PTYPE} and is between 1 and \var{NTYPE}.
...
\begin{table}[h!] \centering
  \begin{tabular}[h]{lll}
    ...
    \bottomrule
  \end{tabular}
  \caption{\var{LTYR} and \var{NMT} values for the \Block{TYR} and \Block{TYRH}.}
  \label{tab:TYR_NMT}
\end{table}

\begin{BlockTable}{TYR}
  \startblock{LTYR}             & \var{TY}$_{1}$         & Particle release for reaction \MT$_{1}$ \\
  \startblock{LTYR}+1           & \var{TY}$_{2}$         & Particle release for reaction \MT$_{2}$ \\
  \ldots \\
  \startblock{LTYR}+\var{NMT}-1 & \var{TY}$_{\var{NMT}}$ & Particle release for reaction \MT$_{\var{NMT}}$
  \label{tab:TYRBlock}
\end{BlockTable}
...
The energy grid index $\var{IE}_{i}$ corresponds to the first energy in the grid at which a cross section is given. The \MT$_{i}$s are defined in the \Block{MTR}.

\subsubsection{\textsf{LAND}, \textsf{LANDP} and \textsf{LANDH} Blocks}\label{sec:LANDBlock}\label{sec:LANDPBlock}\label{sec:LANDHBlock}

The \Block{LAND} (for incident neutron reactions), \Block{LANDP} (for photon production reactions) and \Block{LANDH} (for particle production reactions), give the locators for the angular distribution array for each reaction \MT . A locator is a \emph{relative} index in the \XSS\ array where some piece of data can be found. In this case, the data are the angular distributions. The format of the \Block{LAND}, \Block{LANDP} and \Block{LANDH} is given in \Tableref{tab:LANDBlock} and \Tableref{tab:LANDPBlock}.

The starting index \var{LAND} depends on whether it is the \Block{LAND}, \Block{LANDP} or \Block{LANDH} and is given in \Tableref{tab:LAND_NMT}. For the particle production \Block{LANDH}, \var{i} refers to the index of the corresponding particle type defined on the \Block{PTYPE} and is between 1 and \var{NTYPE}.
...
\begin{table}[h!] \centering
  \begin{tabular}[h]{lll}
    ...
    \bottomrule
  \end{tabular}
  \caption{\var{LAND} and \var{NMT} values for the \Block{TYR} and \Block{TYRH}.}
  \label{tab:LAND_NMT}
\end{table}
...
\begin{BlockTable}{LAND}
  \var{LAND}             & \LOC{B}{1}           & Location of angular distributions for elastic scattering \\
  \var{LAND}+1           & \LOC{B}{2}           & Location of angular distributions for reaction \MT$_{1}$ \\
  \ldots \\
  \var{LAND}+\var{NMT}   & \LOC{B}{\var{NMT}+1} & Location of angular distributions for reaction \MT$_{\var{NMT}}$
  \label{tab:LANDBlock}
\end{BlockTable}

\begin{BlockTable}{LANDP \textnormal{and} LANDH}
  \var{LAND}             & \LOC{B}{1}         & Location of angular distributions for reaction \MT$_{1}$ \\
  \var{LAND}+1           & \LOC{B}{2}         & Location of angular distributions for reaction \MT$_{2}$ \\
  \ldots \\
  \var{LAND}+\var{NMT}-1 & \LOC{B}{\var{NMT}} & Location of angular distributions for reaction \MT$_{\var{NMT}}$
  \label{tab:LANDPBlock}
\end{BlockTable}

\subsubsection{\textsf{AND}, \textsf{ANDP} and \textsf{ANDH} Blocks}\label{sec:ANDBlock}\label{sec:ANDPBlock}\label{sec:ANDHBlock}

The \Block{AND}, \Block{ANDP} and \Block{ANDH} contains angular distribution data for all reactions that produce secondary particles (neutrons for the \Block{AND}, photons for the \Block{ANDP} and a specific particle for the \Block{ANDH}). The format of these blocks is given in \Tableref{tab:ANDBlock} and \Tableref{tab:ANDPBlock}. The angular distribution data begins at the index specified by the locator \LOC{B} from the \Block{LAND}, \Block{LANDP} or \Block{LANDH}. If \LOC{B}{i}=0, no angular distribution data are given for reaction $i$ and isotropic scattering is assumed in either the \LAB\ or \CM\ system. If \LOC{B}{i}=-1 no angular distribution data is given for reaction $i$ (this can only happen for the the \Block{AND} or \Block{ANDH}). In this case, the angular distribution data are specified through \law{44} in the \Block{DLW} or \Block{DLWH}.

The starting index \var{LAND} depends on whether it is the \Block{AND}, \Block{ANDP} or \Block{ANDH} and are given in \Tableref{tab:AND_NMT}. For the particle production \Block{LANDH}, \var{i} refers to the index of the corresponding particle type defined on the \Block{PTYPE} and is between 1 and \var{NTYPE}.
...
\begin{table}[h!] \centering
  \begin{tabular}[h]{lll}
    ...
    \bottomrule
  \end{tabular}
  \caption{\var{LAND} and \var{NMT} values for the \Block{AND} and \Block{ANDH}.}
  \label{tab:AND_NMT}
\end{table}

\begin{ThreePartTable}
\begin{LOCTable}{\Block{AND}}
  \var{LAND}+\LOC{B}{1}-1         & Angular distribution array for elastic scattering \\
  \var{LAND}+\LOC{B}{2}-1         & Angular distribution array for reaction \MT$_{1}$ \\
  \ldots \\
  \var{LAND}+\LOC{B}{\var{NMT}+1}-1 & Angular distribution array for reaction \MT$_{\var{NMT}}$
  \label{tab:ANDBlock}
\end{LOCTable}
\begin{tablenotes}
...
\begin{ThreePartTable}
\begin{LOCTable}{\Block{ANDP} and \Block{ANDH}}
  \var{LAND}+\LOC{B}{1}-1         & Angular distribution array for reaction \MT$_{1}$ \\
  \ldots \\
  \var{LAND}+\LOC{B}{\var{NMT}}-1 & Angular distribution array for reaction \MT$_{\var{NMT}}$
  \label{tab:ANDPBlock}
\end{LOCTable}
\begin{tablenotes}
  \note The format for the angular distribution of the $i$-th array is given in \Tableref{tab:AngularDistributionArray}.
...
\begin{XSSTable}{Angular distribution array for the $i$-th reaction}
  \var{LAND}+\LOC{B}{i}-1     & $N_{E}$                     & Number of energies at which angular distributions are tabulated.  \\
  \var{LAND}+\LOC{B}{i}       & $E(l),l=1,\ldots,N_{E}$     & Energy grid \\
  \ldots \\
  \var{LAND}+\LOC{B}{i}$+N_{E}$ & $L_{C}(l),l=1,\ldots,N_{E}$ & Location of tables associated with $E(l)$
...
\begin{itemize}
  \item if \LOC{C}{l}=0, then distribution is isotropic and no further data is needed;
  \item if \LOC{C}{l}>0, then $\LOC{C}{l}$ points to a 32 equiprobable bin distribution (see \Tableref{tab:32EquiprobableBinDistribution});
  \item if \LOC{C}{l}<0, then $\LOC{C}{l}$ points to a tabulated angular distribution (see \Tableref{tab:TabulatedAngularDistribution}).
\end{itemize}

\begin{XSSTable}{Format for the 32 equiprobable bin distribution}
  \multirow{2}{*}{\var{LAND}+|\LOC{C}{l}|-1} & $P(1,K)$        & 32 equiprobable cosine bins for scattering  \\
...
\begin{XSSTable}{Format for the tabulated angular distribution.}
  \var{LAND}+$|\LOC{C}{l}|-1$        & \var{JJ}                                & Interpolation flag\tnotex{tn:ANDInterpolationFlag} \\
  \var{LAND}+$|\LOC{C}{l}|$          & $N_{P}$                                 & Number of points in the distribution \\
  \var{LAND}+$|\LOC{C}{l}|+1$        & $CS_{\mathrm{out}}(j),j=1,\ldots,N_{P}$ & Cosine scattering angular grid \\
  \var{LAND}+$|\LOC{C}{l}|+1+N_{P}$  & $\PDF(j),j=1,\ldots,N_{P}$              & Probability density function \\
  \var{LAND}+$|\LOC{C}{l}|+1+2N_{P}$ & $\CDF(j),j=1,\ldots,N_{P}$              & Cumulative density function
  \label{tab:TabulatedAngularDistribution}
\end{XSSTable}
\end{ThreePartTable}

The \Block{AND} and \Block{ANDH} can use both options (either a 32 equiprobable bin or tabulated distribution). The \Block{ANDP} on the other hand can only use 32 equiprobable bin distributions.
...
The \Block{LDLW}, \Block{LDLWP} and \Block{LDLWH} give the locators for the energy distribution for every reaction that produces secondary neutron, secondary photons or other secondary particles (respectively). The \Block{DNEDL} on the other hand gives the locators for the delayed neutron energy distribution for each precursor group.

The format of the \Block{LDLW} (for secondary neutrons), the \Block{LDLW} (for secondary photons), the \Block{LDLWH} (for secondary particles)  and the \Block{DNEDL} (for delayed neutrons) is given in \Tableref{tab:LDLWBlock}. The format for the distributions is given in \Sectionref{sec:DLWBlock}.

The starting index \var{LED} depends on whether it is the \Block{LDLW}, \Block{LDLWP}, \Block{LDLWH} or \Block{DNEDL} and are given in \Tableref{tab:LED_NMT}. For the particle production \Block{LDLWH}, \var{i} refers to the index of the corresponding particle type defined on the \Block{PTYPE} and is between 1 and \var{NTYPE}. These blocks are given only if the starting index, \var{LED}, is different from zero.
...
\begin{table}[h!] \centering
  \begin{tabular}{lll}
    ...
    \bottomrule
  \end{tabular}
  \caption{LED and NMT values for the \Block{LDLW}, the \Block{LDLWP}, the \Block{LDLWH} and \Block{DNEDL}.}
  \label{tab:LED_NMT}
\end{table}
...
\begin{table}[h!] \centering
  \begin{tabular}{lll}
    ...
    \bottomrule
  \end{tabular}
  \caption{\var{JED} and \var{NMT} for the \Block{DLW}, \Block{DLWP} and \Block{DLWH}.}
  \label{tab:JED_NMT}
\end{table}
...
\begin{ThreePartTable}
  \begin{TableNotes}
    \item[$\dagger$] \label{tn:LAW66InterpolationScheme}
      \begin{description}
        \item[\var{INTT}=1] histogram distribution,
        \item[\var{INTT}=2] linear-linear distribution.
      \end{description}
  \end{TableNotes}
  \begin{LAWTable}{\var{LAW}=66 (From ENDF-6 \MF=6 \var{LAW}=6)}
    ...
    \var{LDAT}(3)          & \var{INTT}                                & Interpolation parameter\tnotex{tn:LAW66InterpolationScheme} \\
    \var{LDAT}(4)          & $N_{P}$                                   & Number of points in the distribution \\
    \var{LDAT}(5)          & $\xi_{\mathrm{out}}(j),j=1,\ldots,N_{P}$  & $\xi$ grid (between 0 and 1) \\
...
While MCNP will only use the values for \var{NPSX} and $A_{P}$ (given in \var{LDAT}(1) and \var{LDAT}(2)), NJOY does add the derived probability density function and cumulative density function for $T(\xi)$ starting at \var{LDAT}(3).
...
\subsubsection{\textsf{SIGP} and \textsf{SIGH} Blocks}\label{sec:SIGPBlock}\label{sec:SIGHBlock}

The \Block{SIGP} contains the photon production cross section data and the \Block{SIGH} contains the particle production cross section data. The format of the \Block{SIGP} and \Block{SIGH} is given in \Tableref{tab:SIGPBlock}. The starting index depends on whether it is the \Block{LSIGP} or \Block{LSIGH} and are given in \Tableref{tab:SIG_NMT}. For the particle production \Block{SIGH}, \var{i} refers to the index of the corresponding particle type defined on the \Block{PTYPE} and is between 1 and \var{NTYPE}.
...
\begin{table}[h!] \centering
  \begin{tabular}[h]{lll}
    ...
    \bottomrule
  \end{tabular}
  \caption{\var{SIG} and \var{NMT} values for the \Block{SIGP} and \Block{SIGH}.}
  \label{tab:SIG_NMT}
\end{table}

The cross section data begins at the index specified by the locator, \LOC{A}{i}, given in the \Block{LSIGP} or \Block{LSIGH}. The \MT s are defined in the \Block{MTRP} or \Block{MTRH}. All indices to the \XSS\ array are \emph{relative} to \var{SIG}.

\begin{BlockTable}{SIGP}
  \var{SIG}+\LOC{A}{1}-1         & \var{MFTYPE}$_{1}$ & Cross section array for reaction \MT$_{1}$ \\
  \var{SIG}+\LOC{A}{2}-1         & \var{MFTYPE}$_{2}$ & Cross section array for reaction \MT$_{2}$ \\
  \ldots \\
  \var{SIG}+\LOC{A}{\var{NMT}}-1 & \var{MFTYPE}$_{\var{NMT}}$ & Cross section array for reaction \MT$_{\var{NMT}}$
  \label{tab:SIGPBlock}
\end{BlockTable}

The format of the $i$-th cross section array has two possible forms depending on the first number in the array, \var{MFTYPE}.
\begin{enumerate}
  \item If \var{MFTYPE}=12 or \var{MFTYPE}=16, yield data taken from ENDF File 12 or 6, respectively (see \Tableref{tab:PhotonProductionArray}). With this format, the photon or particle production cross section can be constructed using \Equationref{eq:PhotonProductionConstruction}:
  \begin{equation}
    \sigma_{i}(E) = Y(E)*\sigma_{\var{MTMULT}}(E).
    \label{eq:PhotonProductionConstruction}
  \end{equation}
  \item If \var{MFTYPE}=13, photon production cross section data from ENDF File 13 (see \Tableref{tab:PhotonProductionCrossSectionArray}). This form is only allowed for the \Block{SIGP}.
\end{enumerate}
...
\subsubsection{\textsf{YP} and \textsf{YH} Blocks}\label{sec:YPBlock}\label{sec:YHBlock}

The \Block{YP} and \Block{YH} contains a list of \MT\ identifiers of cross sections that are used as yield multipliers in \Equationref{eq:PhotonProductionConstruction} to calculate the photon production cross sections (for the \Block{YP}) and the secondary particle production cross sections (for the \Block{YH}) and are referenced by the \var{MTMULT} parameter in \Tableref{tab:PhotonProductionArray}. The format of the \Block{YP} and \Block{YH} is given in \Tableref{tab:YPBlock}.

The starting index \var{LY} depends on whether it is the \Block{YP} or \Block{YH}. For the \Block{YP}, \var{LY} = \nxs{6}. For the particle production \Block{YH}, \var{JED} = \xss{\jxs{32}+10*(i-1)+8} in which \var{i} refers to the index of the corresponding particle type defined on the \Block{PTYPE} and is between 1 and \var{NTYPE}. These blocks are given only if the starting index \var{LY} is different from zero.

\begin{BlockTable}{YP}
  \var{LY}   & \var{NYP} & Number of \MT s to follow \\
  \var{LY}+1 & $\var{MTY}(l),l=1,\ldots,\var{NYP}$ & \MT s.
  \label{tab:YPBlock}
\end{BlockTable}
...
\begin{XSSTable}{Order of probability table elements $P(i,j,k)$}
  \multicolumn{3}{c}{\bfseries Data for $\mathbf{E(1)}$} \\
  \var{PTABLE}         & $\CDF_{1}(l),l=1,\ldots,M$              & Cumulative probabilities\tnotex{tn:CumulativeProbabilities}\ for energy $i=1$ \\
  \var{PTABLE}+$M$     & $\sigma_{t,1}(l),l=1,\ldots,M$          & Total cross section/factors for energy $i=1$ \\
  \var{PTABLE}+$2M$    & $\sigma_{s,1}(l),l=1,\ldots,M$          & Elastic cross section/factors for energy $i=1$ \\
  \var{PTABLE}+$3M$    & $\sigma_{f,1}(l),l=1,\ldots,M$          & Fission cross section/factors for energy $i=1$ \\
  \var{PTABLE}+$4M$    & $\sigma_{(n,\gamma),1}(l),l=1,\ldots,M$ & $(n,\gamma)$ cross section/factors for energy $i=1$ \\
  \var{PTABLE}+$5M$    & $H_{1}(l),l=1,\ldots,M$                 & Heating number/factors for energy $i=1$ \\
  \cmidrule{1-3}
  \multicolumn{3}{c}{\bfseries Data for incident energy 2---same format for $E(1)$} \\
  \multicolumn{3}{c}{\ldots} \\
  \multicolumn{3}{c}{\bfseries Data for incident energy $N$---same format for $E(1)$}
...
The \Block{PTYPE} is the first of the particle production blocks used for neutron and charged particle production data. These particle production blocks are given only if the number of particles \var{NTYPE} = \nxs{7} is different from zero. If the \Block{PTYPE} is present, the \Block{PTYPE} starts at the index \var{LTYPE} = \jxs{30}.

{\color{red}\textbf{Question:} The particle type can be neutron, when is this actually used?}

The \Block{PTYPE} gives a list of particle types for which particle production data is available. This includes cross section data (given in the \Block{SIGH}), angular distribution data (given in the \Block{ANDH}) and secondary particle energy distribution data (given in the \Block{DLWH}).

The format of the \Block{PTYPE} is given in \Tableref{tab:PTYPEBlock}.

\begin{BlockTable}{PTYPE}
  \var{LTYPE}               & \var{IP}$_{1}$         & First particle type \\
  \var{LTYPE}+1             & \var{IP}$_{2}$         & Second particle type \\
  \ldots \\
  \var{LTYPE}+\var{NTYPE}-1 & \var{IP}$_{\var{NMT}}$ & Last particle type
...
The \Block{NTRO} gives the number of reactions for each of the particle types defined in the \Block{PTYPE}. If the \Block{NTRO} is present, the \Block{NTRO} starts at the index \var{LTYPE} = \jxs{31}.

The format of the \Block{NTRO} is given in \Tableref{tab:NTROBlock}.

\begin{BlockTable}{NTRO}
  \var{LTYPE}               & \var{NP}$_{1}$         & Number of reactions producing the first particle type \\
  \var{LTYPE}+1             & \var{NP}$_{2}$         & Number of reactions producing the second particle type \\
  \ldots \\
  \var{LTYPE}+\var{NTYPE}-1 & \var{NP}$_{\var{NMT}}$ & Number of reactions producing the last particle type
  \label{tab:NTROBlock}
\end{BlockTable}

\subsubsection{\textsf{IXS} Block}\label{sec:IXSBlock}

The \Block{IXS} gives 10 particle production locators for each of the particle types defined in the \Block{PTYPE}. If the \Block{IXS} is present, the \Block{IXS} starts at the index \var{NEXT} = \jxs{32}. The \Block{IXS} serves a similar function as the \hyperref[sec:JXSContinuousEnergyNeutron]{JXS Array}, in that it provides the locators to specific blocks of data, as laid out in \Tableref{tab:IXSBlock}.

\begin{BlockTable}{IXS array for particle type $j$, with $\var{LTYPE}=\var{NEXT}+10\var{NTYPE}(j-1)$}
  \var{LTYPE}     & \var{HPD}    & Location of the total particle production and heating data \\
  \var{LTYPE}+1   & \var{MTRH}   & Location of the particle production MT array \\
  \var{LTYPE}+2   & \var{TYRH}   & Location of the particle production TYR data \\
  \var{LTYPE}+3   & \var{LSIGH}  & Location of the particle production cross section locators \\
  \var{LTYPE}+4   & \var{SIGH}   & Location of the particle production cross sections \\
  \var{LTYPE}+5   & \var{LANDH}  & Location of the particle production angular distribution locators \\
  \var{LTYPE}+6   & \var{ANDH}   & Location of the particle production angular distributions \\
  \var{LTYPE}+7   & \var{LDLWH}  & Location of the particle production energy distribution locators \\
  \var{LTYPE}+8   & \var{DLWH}   & Location of the particle production energy distributions \\
  \var{LTYPE}+9   & \var{YH}     & Location of the particle production yield multipliers
  \label{tab:IXSBlock}
\end{BlockTable}

With the exception of the \Block{HPD}, all other locators point to blocks similar to the ones already defined for neutrons and photons.

\subsubsection{\textsf{HPD} Block}\label{sec:HPDBlock}

The \Block{HPD} gives the total particle production cross section and the associated heating number for a given particle. If the particle production data is given (i.e. the $\var{NTYPE}\ne0$, and the \Block{PTYPE}, the \Block{NTRO} and \Block{IXS} are given), the \Block{HPD} is present. The \Block{HPD} starts at the location index $\var{HPD}=\xss(\var{NEXT}+10*(j-1))$.

\begin{BlockTable}{HPD}
  \var{HPD}              & \var{IE}                         & Energy grid index \\
  \var{HPD}+1            & $N_{E}$                          & Number of consecutive energies \\
  \var{HPD}+2            & $\sigma[E(K)],$                  & Total particle production cross section \\
  \var{HPD}+2+\var{NE}   & $E(l),l=1,\ldots,N_{E}$          & Average heating numbers \\
  \label{tab:HPDBlock}
\end{BlockTable}
