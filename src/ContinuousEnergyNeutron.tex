...
The \Block{LAND} (for incident neutron cross sections), \Block{LANDP} (for photon production cross sections) and \Block{LANDH} (for particle production cross sections) give the locators for the angular distribution array for each reaction \MT . A locator is a \emph{relative} index in the \XSS\ array where some piece of data can be found. In this case, the data are the angular distributions. The format of the \Block{LAND}, \Block{LANDP} and \Block{LANDH} is given in \Tableref{tab:LANDBlock}.
...
\subsubsection{\textsf{AND} and \textsf{ANDH} Block}\label{sec:ANDBlock}

The \Block{AND} and \Block{ANDH} contains angular distribution data for all reactions that produce secondary particles (neutrons for the \Block{AND} and a specific particle for the \Block{ANDH}). The format of these blocks is given in \Tableref{tab:ANDBlock}. The angular distribution data begins at the index specified by the locator \LOC{B} from the \Block{LAND} or \Block{LANDH}. If \LOC{B}{i}=0 (given in the \Block{LAND} and \Block{LANDH}), no angular distribution data are given for reaction $i$ and isotropic scattering is assumed in either the \LAB\ or \CM\ system. The choice of \LAB\ or \CM\ system depends upon the value for reaction $i$ in the \Block{TYR} and \Block{TYRH}. If \LOC{B}{i}=-1 no angular distribution data are given for reaction $i$ in the \Block{AND} or \Block{ANDH}. The angular distribution data are specified through \law{44} in the \Block{DLW} or \Block{DLWH}. 

The starting index \var{LAND} depends on whether it is the \Block{AND} or \Block{ANDH} and are given in \Tableref{tab:LAND_NMT}. For the particle production \Block{LANDH}, \var{i} refers to the index of the corresponding particle type defined on the \Block{PTYPE} and is between 1 and \var{NTYPE}.
...
\subsubsection{\textsf{LANDP} and \textsf{LANDH} Block}\label{sec:LANDPBlock}\label{sec:LANDHBlock}

The \Block{LANDP} (for photon production) and \Block{LANDH} (for particle production) give the locators for the angular distribution array for each reaction \MT. A locator is a \emph{relative} index in the \XSS\ array where some piece of data can be found. In this case, the data are the angular distributions. The format of the \Block{LANDP} and \Block{LANDH} is given in \Tableref{tab:LANDPBlock}. The angular distribution format for photon production is given in \Sectionref{sec:ANDPBlock} and the format for particle production is given in \Sectionref{sec:ANDHBlock}.

The starting index \var{LAND} depends on whether it is the \Block{LANDP} or \Block{LANDH} and are given in \Tableref{tab:LAND_NMT}. For the particle production \Block{LANDH}, \var{i} refers to the index of the corresponding particle type defined on the \Block{PTYPE} and is between 1 and \var{NTYPE}.
...
The \MT s are given in the the \Block{MTRP} and \Block{MTRH} respectively for the \Block{LANDP} and \Block{LANDH} respectively. \LOC{B}{i} must be monotonically increasing. All locators (\LOC{B}) are \emph{relative} to \var{ANDP}=\jxs{17} for the \Block{LANDP} and relative to \var{ANDH}=\xss{\jxs{32}+10*(i-1)+6} for the \Block{LANDP}.

\begin{BlockTable}{LSIG \textnormal{\&} LSIGP}
  \var{LAND}             & \LOC{B}{1}         & Location of cross sections for reaction \MT$_{1}$ \\
  \var{LAND}+1           & \LOC{B}{2}         & Location of cross sections for reaction \MT$_{2}$ \\
  \ldots \\
  \var{LAND}+\var{NMT}-1 & \LOC{B}{\var{NMT}} & Location of cross sections for reaction \MT$_{\var{NMT}}$
  \label{tab:LSIGBlock}
\end{BlockTable}
