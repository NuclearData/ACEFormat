...
\subsubsection{\textsf{NU} \& \textsf{DNU} Block}\label{sec:NUBlock}\label{sec:DNUBlock}
...
\begin{ThreePartTable}
  ...
  \begin{XSSTable}{Delayed \nubar\ precursor distribution.}
    \var{KNU}                   & \var{DEC}$_{i}$                  & Decay constant for the $i$-th group \\
    \var{KNU}+1                 & $N_{R}$                          & Number of interpolation regions \\
  ...
  \end{XSSTable}
  \begin{tablenotes}
    \note \var{KNU} is the index of the \XSS\ array where the delayed \nubar\ precursor data for each group begins; the first one is at \var{DNU} = \jxs{25}.
  \end{tablenotes}
\end{ThreePartTable}

The \Block{BDD} is followed by the \Block{DNEDL} and the \Block{DNED} blcoks which gives the locators (\Block{DNEDL}) and the delayed neutron energy distributions (\Block{DNEDL}) for every delayed neutron precursor group.
...
\subsubsection{\textsf{LDLW} \& \textsf{LDLWP} \& \textsf{DNEDL} Blocks}\label{sec:LDLWBlock}\label{sec:LDLWPBlock}\label{sec:DNEDLBlock}
