...
\begin{enumerate}
  \item \textbf{\Block{ESZ}}---contains the main energy gid for the Table and the total, absorption, and elastic cross sections as well as the average heating numbers. The \Block{ESZ} block always exists. See \Sectionref{sec:ESZBlock}.
...
\todo[inline]{Which of these formats really need to still be supported? ENDF has stopped supporting a number of these 30+ years ago.}
...
\end{description}
\todo[inline]{This looks slightly different than the one in the ENDF-6 document. It may be similar, but we should double check.}
...
\begin{LAWTable}{Secondary energy distribution for each incident energy in \var{LAW}=44}
  ...
  \var{LDAT}(\var{K}+$2+N_{p}$)  & $\mathrm{PDF}(l),l=1,\ldots,N_{p}$     & Probability Density Function \\
  \var{LDAT}(\var{K}+$2+2N_{p}$) & $\mathrm{CDF}(l),l=1,\ldots,N_{p}$     & Cumulative Density Function \\
  \var{LDAT}(\var{K}+$2+3N_{p}$) & $R(l),l=1,\ldots,N_{p}$                & Precompound fraciton $r$ \\
...
\begin{equation}
  ...
  \label{eq:LAW44p}
\end{equation}
\todo[inline]{Is this equation specific to the format or to MCNP?}

\subsubsubsection{\var{LAW}=61---Like \var{LAW}=61, but tabular angular distribution instead of Kalbach-87}\label{sec:LAW61}
\begin{ThreePartTable}
  \begin{TableNotes}
...
\begin{ThreePartTable}
  ...
  \begin{LAWTable}{Angular distribution for \var{LAW}=67}
    ...
    \var{LDAT}(\var{K}+2) & $\var{XMU}(l),l=1,\ldots,\var{NMU}$ & Secondary cosins \\
