...
\begin{XSSTable}{Cross section array for the $i$-th reaction.}
  ...
  \multirow{2}{*}{\var{LXS} + \LOC{A}{i}+1} & $\sigma_{i}[E(l)]$ for                         & \multirow{2}{*}{Cross section for reaction \MT$_{i}$} \\
...
\subsubsection{\Block{LDLW \textnormal{\&} LDLWP}}\label{sec:LDLWBlock}
...
\begin{ThreePartTable}
  ...
  \item[$\ast$] \label{tn:EnergyDistributionProbability} If the particle energy $E<$(1), then $P(E)=P(1)$. If $E>E(N_{E})$, then $P(E)=P(N_{E})$. If more than one law is given, then \var{LAW}$_{1}$ is used only if $\xi<P(E)$ where $\xi$ is a random number between 0 and 1.
  \end{TableNotes}
...
\begin{equation}
  E_{\mathrm{out}}^{\mathrm{LAB}} = E_{\mathrm{out}}^{\mathrm{CM}} +\left\{ E+2\mu_{\mathrm{CM}}(A+1)(EE_{\mathrm{out}}^{\mathrm{CM}})^{1/2} \right\}/(A+1)^{2}
  \label{eq:Law3EoutLAB}
\end{equation}
where $\mu_{\mathrm{CM}}$ is the cosine of the \CM\ scattering angle
\todo[inline]{What is $EE_{\mathrm{out}}^{\mathrm{CM}}$?}
...
\subsubsubsection{\var{LAW}=5---General Evaporation Spectrum}\label{sec:LAW5}
  ...
  \begin{LAWTable}{\var{LAW}=5 (From ENDF-6, \MF=5, \var{LF}=5)}
    \var{LDAT}(1)                 & $N_{R}$                          & \multirow{2}{*}{Interpolation scheme between $T$'s} \\
    ...
    \var{LDAT}(3+$2N_{R}+N_{E}$)  & $T(l),l=1,\ldots,N_{E}$          & Tabulated function of incident energies \\
  ...
  \end{LAWTable}
  \begin{tablenotes}
    \note $E_{\mathrm{out}} = X(\xi)T(E)$ where $X(\xi)$ is a randomly sampled table of $X$'s and $E$ is the incident energy.
  \end{tablenotes}
\end{ThreePartTable}
...
\begin{LAWTable}{\var{LAW}=7 (From ENDF-6, \MF=5, \var{LF}=7)}
    \var{LDAT}(1)                 & $N_{R}$                          & \multirow{3}{*}{Interpolation scheme between $T$'s} \\
    \var{LDAT}(2)                 & \var{NBT}$(l), l=1,\ldots,N_{R}$ & \\
    \var{LDAT}(2+$N_{R}$)         & \var{INT}$(l), l=1,\ldots,N_{R}$ & \\
    \cmidrule{1-3}
    \var{LDAT}(2+$2N_{R}$)        & $N_{E}$                          & Number of incident energies tabulated \\
...
\begin{equation}
  f(E\rightarrow E_{\mathrm{out}}) = C\sqrt{E_{\mathrm{out}}}\ e^{-E_{\mathrm{out}}/T(E)}
  \label{eq:LAW7f}
\end{equation}
with restriction $0 \leq E_{\mathrm{out}} \leq E-U$
\begin{equation}
  C = T^{{-3/2}} \left[ \frac{\sqrt{\pi}}{2} \erf\left( \sqrt{(E-U)/T} \right) +-\sqrt{(E-U)/T}\ e^{-(E-U)/T} \right]^{-1}
  \label{eq:LAW7C}
\end{equation}
\todo[inline]{\Equationref{eq:LAW7C} doesn't look correct. The $+-$ in the middle seems suspicious.}
...
with restriction $0 \leq E_{\mathrm{out}} \leq E-U$
\begin{equation}
  C = T^{{-3/2}} \left[ t-e^{(E-U)/T}\left( 1+(E-U)/T \right) \right]^{1/2}
  \label{eq:LAW9C}
\end{equation}
\todo[inline]{These equations should also be double-checked.}
...
\begin{LAWTable}{\var{LAW}=11 (From ENDF-6, \MF=5, \var{LF}=11)}
  \var{LDAT}(1)                        & $N_{R_{a}}$                              & \multirow{3}{*}{Interpolation scheme between $a$'s} \\
  ...
  \var{LDAT}(\var{L})                          & $N_{R_{b}}$                              & \multirow{3}{*}{Interpolation scheme between $b$'s} \\
...
\begin{equation}
  f\left( E\rightarrow E_{\mathrm{out}} \right) = C_{o}e^{-E_{\mathrm{out}}/a(E)}\sinh [b(E)E_{\mathrm{out}}]^{1/2}
  \label{eq:LAW11f}
\end{equation}
with restriction $0\leq E_{\mathrm{out}}<E-U$.

\subsubsubsection{\var{LAW}=22---Tabular Linear Functions}\label{sec:LAW22}
\begin{LAWTable}{\var{LAW}=22 (From UK Law 2)}
    \var{LDAT}(1)                 & $N_{R}$                          & \multirow{3}{*}{Interpolation parameters} \\
    \var{LDAT}(2)                 & \var{NBT}$(l), l=1,\ldots,N_{R}$ & \\
    \var{LDAT}(2+$N_{R}$)         & \var{INT}$(l), l=1,\ldots,N_{R}$ & \\
    \cmidrule{1-3}
    \var{LDAT}(2+$2N_{R}$)        & $N_{E}$                          & Number of incident energies tabulated \\
    \var{LDAT}(3+$2N_{R})$ & $E_{\mathrm{in}}(l),l=1,\ldots,N_{E}$ & Number of incdident energies for $E_{\mathrm{out}}$ tables \\
    ...
    \var{LDAT}(\var{L}+1)                 & $P_{1}(K),K=1,\ldots,\var{NF}_{1}$ \\
    \var{LDAT}(\var{L}+1+\var{NF}$_{1}$)  & $T_{1}(K),K=1,\ldots,\var{NF}_{1}$ \\
    \var{LDAT}(\var{L}+1+2\var{NF}$_{1}$) & $C_{1}(K),K=1,\ldots,\var{NF}_{1}$ \\
    \cmidrule{1-3}
    \multicolumn{3}{l}{Data for $E_{\mathrm{in}}(2)$:} \\
    \ldots
  \label{tab:LAW22}
\end{LAWTable}
\todo[inline]{The following equations seem very wrong, but they are no better in the original.}
If
\begin{equation}
  E_{\mathrm{in}}(l) \leq E < E_{\mathrm{in}}(l+1)
\end{equation}
then, for a given random number, $\xi\in[0,1)$, if 
\begin{equation}
  \sum_{k=1}^{k=K} P_{i} < \xi \leq \sum_{k=1}^{k=K}P_{i}(k)
  \label{eq:LAW22Sum}
\end{equation}
then 
\begin{equation}
  E_{\mathrm{out}} = C_{i}(K)\left( E-T_{i}(K) \right).
  \label{eq:LAW22Eout}
\end{equation}
...
\begin{LAWTable}{\var{LAW}=61---Like \var{LAW}=44, but tabular angular distribution instead of Kalbach-87}
  \label{tab:LAW61}
\end{LAWTable}
...
The format of the this Block is given in \Tableref{tab:GPDBlock}. The \XSS\ array index at the start of the \Block{GPD}, \startblock{GPD}=\jxs{12}.
\begin{BlockTable}{GPD}
  ...
  \ldots \\
  \startblock{GPD}+\var{NES}+(30-1)*20 & $E_{2}(K),K=1,20$ & \num{20} equiprobable outgoing photon energies for incident neutron $E \geq E_{N}(30)$
  \label{tab:GPDBlock}
\end{BlockTable}

\subsubsection{\Block{SIGP}}\label{sec:SIGPBlock}
The \Block{SIGP} contains the photon production cross section data. The format of the \Block{SIGP} is given in \Tableref{tab:SIGPBlock}. The cross section data begins at the index specified by the locator, \LOC{A}{i}, given in the \Block{LSIG} (see \Sectionref{sec:LSIGBlock}). All indices to the \XSS\ array are \emph{relative} to \jxs{15}. 
\begin{ThreePartTable}
...
\todo[inline]{Should this section be merged with \Sectionref{sec:LANDBlock}? It is just a minor difference. The combination is done similarly in \Sectionref{sec:LDLWBlock}.}
