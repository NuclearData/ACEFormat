...
The \LOC{A}{i} values are given in the \Block{LSIG} and are all relative to \jxs{7}. The energy grid, $E(K)$ is given in the \Block{ESZ}. The energy grid index $IE_{i}$ corresponds to the first energy in the grid at which a cross section is given. The \MT$_{i}$s are defined in the \Block{MTR}.
\begin{XSSTable}{Cross section array for the $i$-th reaction.}
\var{LXS} + \LOC{A}{i}-1                  & $IE_{i}$                          & Energy grid index for reaction \MT$_{i}$ \\
\var{LXS} + \LOC{A}{i}                    & $NE_{i}$                          & Number of consecutive entries for \MT$_{i}$ \\
\multirow{2}{*}{\var{LXS} + \LOC{A}{i}+1} & $\sigma_{i}[E(K)]$ for            & \multirow{2}{*}{Cross section for reaction \MT$_{i}$} \\
...
\begin{XSSTable}{Photon production array if \var{MFTYPE}=12 or 16}
  ...
  \jxs{15}+\LOC{A}{i}+2   & \var{NBT}$(l), l=1,\ldots,N_{R}$                  & ENDF interpolation parameters \\
